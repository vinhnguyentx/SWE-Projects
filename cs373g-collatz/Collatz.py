...
#         tile[i] = collatz_eval(i * 1000, i * 1000 + 1000)
...
def collatz_read(string):
    ...
    """
    inString = string.split()
    return [int(inString[0]), int(inString[1])]
...
def cycle_length(num):
    """
    read one int
    return the cycle_length of the int
    """
    assert num > 0
    ...
    if num == 1:
        ...
        return cache[num]
    elif num & 1 == 0 and cache[num >> 1] != 0:
        cache[num] = cache[num >> 1] + 1
        return cache[num >> 1] + 1
    elif num & 1 != 0 and cache[num + (num >> 1) + 1] != 0:
        cache[num] = cache[num + (num >> 1) + 1] + 2
        return cache[num + (num >> 1) + 1] + 2
    else:
        len = 1
        temp = num
        while temp > 1:
...
#------------------------
# collatz_eval_meta_cache
#------------------------
def collatz_eval_meta_cache(start, end):
    """
    start: the beginning of the range, inclusive
    end: the end of the range, inclusive
    return the max cycle length of the range [start, end] using a meta-cache
    """
    assert start > 0
    assert end > 0
    ...
    start_cache = start // 1000 + 1
    end_cache = end // 1000
    left_bound = (start, min(start_cache * 1000, end))
    right_bound = (min(end_cache * 1000, end), end)

    left_bound_start = start
    left_bound_end = min(start_cache * 1000, end)
    right_bound_start = min(end_cache * 1000, end)
    right_bound_end = end
    ...
    if end <= 1000:
        return collatz_eval(start, end)

    if end_cache > start_cache:
        for n in range(start_cache, end_cache):
            if meta_cache[n] > max_cache:
                max_cache = meta_cache[n]

    if left_bound_end > left_bound_start:
        max_left_bound = collatz_eval(left_bound_start, left_bound_end)

    if right_bound_end > right_bound_start:
        max_right_bound = collatz_eval(right_bound_start, right_bound_end)

    return max(max_cache, max_left_bound, max_right_bound)
    ...
    for n in range(start, end + 1):
        if n & 1 == 0:
            if cache[n >> 1] != 0:
                ...
                len = cycle_length(n >> 1) + 1
        else:
            if cache[n + (n >> 1) + 1] != 0:
                len = cache[n + (n >> 1) + 1] + 2
            else:
                len = cycle_length(n + (n >> 1) + 1) + 2
...
# collatz_print
# -------------
def collatz_print(writer, start, end, value):
    """
    print three ints
    w a writer
    ...
    """
    writer.write(str(start) + " " + str(end) + " " + str(value) + "\n")
...
def collatz_solve(reader, writer):
    ...
    for s in reader:
        ...
        if s is not "":
            i, j = collatz_read(s)
            # if start of the range is greater than end, then switch
            if i > j:
                temp = i
                i = j
                j = temp

                v = collatz_eval_meta_cache(i, j)
                # print reversely to get the right order of start - end of the range
                collatz_print(writer, j, i, v)
            else:
                v = collatz_eval_meta_cache(i, j)
                collatz_print(writer, i, j, v)
        else:
            continue
